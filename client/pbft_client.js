...
function sendMessage(node, message) {
  ...
  client.on('error', (err) => {
    console.error(`Error connecting to ${node.hostname}:${node.port}: ${err.message}`);
    client.destroy(); // Close the connection
  });
}
  ...
  rl.on('line', (input) => {
    ...
    if (command === 'send' && args[0] === 'transaction') {
      ...
      if (from && to && !isNaN(amount)) {
        ...
        console.error('Invalid transaction command. Usage: send transaction <from> <to> <amount>');
      }
      ...
      console.error('Invalid command. Available commands: send transaction <from> <to> <amount>, get state');
    }
